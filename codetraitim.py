...
pygame.display.set_caption("Heart Ly Tuan")
...
c = np.linspace(-2, -2.4, size_steps)
d = np.linspace(-1, -0.16, size_steps)
scalex = np.linspace(1, 1.18, size_steps) * 15
scaley = np.linspace(1, 1.36, size_steps) * 15
...
for repeat in range(3):
    for t in np.concatenate((np.linspace(0.18, 3.14-0.18, size_steps//1), np.linspace(3.14+0.18, 2*3.1415-0.18, size_steps//1)), axis=0):
        off_s = -np.random.exponential(1.8)
        size = int(rand(1.5, 2.5))
...
for repeat in range(3):
    for t in np.concatenate((np.linspace(0.2, 3.14-0.2, size_steps//1), np.linspace(3.14+0.2, 2*3.1415-0.2, size_steps//1)), axis=0):
        off_x = np.random.randn() * 2
        off_y = np.random.randn() * 2
        off_s = np.random.randn() * 1.5 - 1.8
        size = int(rand(1.5, 2.5))
...
#     def draw(self, screen: pygame.Surface, i, heartbeat_factor):
#         x, y = self.get_pos(i)
#         size = int(self.size * heartbeat_factor)
#         draw_circle(screen, x, y, size, self.color)
...
#             heartbeat_factor = np.sin(frame / 20) * 0.2 + 1.0

#             for p in persistent_particles:
#                 p.draw(screen, index, heartbeat_factor)
